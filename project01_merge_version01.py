import sys
from PyQt5.QtWidgets import QApplication, QWidget, QHBoxLayout, QVBoxLayout, QFrame, QTableWidget, QTableWidgetItem, \
...
class TotalIncomeTax(Tax):
    def calculation(self):
        if self.netProfit <= 12000000.0:
            ...
            return (self.netProfit - 88000000.0) * 0.35 + (88000000.0 - 46000000.0) * 0.24 + (
                        46000000.0 - 12000000.0) * 0.15 + 12000000.0 * 0.06
        elif self.netProfit <= 500000000.0:
            return (self.netProfit - 150000000.0) * 0.38 + (150000000.0 - 88000000.0) * 0.35 + (
                        88000000.0 - 46000000.0) * 0.24 + (46000000.0 - 12000000.0) * 0.15 + 12000000.0 * 0.06
        else:
            return (self.netProfit - 500000000.0) * 0.40 + (500000000.0 - 150000000.0) * 0.38 + (
                        150000000.0 - 88000000.0) * 0.35 + (88000000.0 - 46000000.0) * 0.24 + (
                        46000000.0 - 12000000.0) * 0.15 + 12000000.0 * 0.06
...
def on_store_select(item):
    ...
    bottom_label.setStyleSheet("color: black; font-size: 15px;")
...
if __name__ == "__main__":
    ...
    for storeUrl in crawlingList:
        ...
        if fileCreated == False: # 첫번째 줄은 파일 초기화
            fileCreated = True
    ...
    def button_clicked():
        left_table.clicked.disconnect(on_store_select)
    ...
    with open(csvName, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            tempList = [row[0], int(row[1]), int(row[2]), int(row[3]), int(row[4]), int(row[5]), int(row[6]),
                        int(row[7]), int(row[8])]
